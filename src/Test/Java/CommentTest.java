...
public class FoodTest {
    ...
    public void insertData() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if ((i / 20) == 0 || i == 0) {
                ...
                user.setPhoneNumber("187 1000" + i);
                user.setName("测试用户" + i);
    ...
    public void queryKey(){
        ...
        if (Message.RESULT_OK == jsonObject.get("status_code").getAsInt()){
            ...
            List<QueryData<List<KeySearchResult>>> re = gson.fromJson(data,new TypeToken<List<QueryData<List<KeySearchResult>>>>(){}.getType());
    ...
    @org.junit.Test
    public void getDistanceJsonTest() {
        DistanceQueryResult distanceQueryResult = new DistanceQueryResult();
        distanceQueryResult.setInfo("OK");
        distanceQueryResult.setInfoCode("10000");
        distanceQueryResult.setStatus("1");

        DistanceResultItem distanceResultItem = new DistanceResultItem();
        distanceResultItem.setDestId("1");
        List<DistanceResultItem> items = new ArrayList<DistanceResultItem>();
        items.add(distanceResultItem);

        distanceQueryResult.setResults(items);
        Gson gson = new GsonBuilder().serializeNulls().create();
        Print.msg(gson.toJson(distanceQueryResult));
